...
Unified logging configuration for the entire DeepHSCode system.
...
def get_default_log_dir() -> Path:
    ...
    from DeepHSCode.services.config import PROJECT_ROOT
...
def load_logging_config() -> LoggingConfig:
    ...
    try:
        ...
        # If log_dir is specified in config, use it; otherwise default to PROJECT_ROOT / data / user / logs
        configured_log_dir = logging_config.get("log_dir")
        if configured_log_dir:
            log_dir = configured_log_dir
        else:
            log_dir = str(PROJECT_ROOT / "data" / "user" / "logs")
