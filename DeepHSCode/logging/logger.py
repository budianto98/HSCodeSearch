...
class Logger:
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(f"DeepHSCode.{name}")
        ...
        log_dir_path: Path
        if log_dir is None:
            # Default to PROJECT_ROOT / data / user / logs
        ...
        if file_output:
            ...
            log_file = log_dir_path / f"hscode_{timestamp}.log"
...
def get_logger(
    ...
    if log_dir is None or effective_level is None:
        try:
            from DeepHSCode.services.config import get_path_from_config, load_config_with_main
