...
from DeepHSCode.services.config import PROJECT_ROOT, parse_language
...
class PromptManager:
    ...
    # Modules that are not under DeepHSCode/agents/ directory
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PROJECT_ROOT / "DeepHSCode" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PROJECT_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PROJECT_ROOT / "DeepHSCode" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]
