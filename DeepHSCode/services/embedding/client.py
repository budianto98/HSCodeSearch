...
_BINDING_TO_BACKEND: dict[str, str] = {
...
def _resolve_adapter_class(binding: str) -> type[BaseEmbeddingAdapter]:
    provider = (binding or "openai").strip().lower()
    backend = _BINDING_TO_BACKEND.get(provider)
    if backend is None:
        supported = sorted(_BINDING_TO_BACKEND.keys())
        raise ValueError(
            f"Unknown embedding binding: '{binding}'. Supported: {', '.join(supported)}"
        )
    cls = _ADAPTER_MAP.get(backend)
    if cls is None:
        raise ValueError(f"No adapter registered for backend '{backend}' (binding='{binding}')")
    return cls
