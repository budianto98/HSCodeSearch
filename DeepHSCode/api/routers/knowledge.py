...
from DeepHSCode.api.utils.progress_broadcaster import ProgressBroadcaster
from DeepHSCode.api.utils.task_id_manager import TaskIDManager
from DeepHSCode.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from DeepHSCode.knowledge.add_documents import DocumentAdder
from DeepHSCode.knowledge.initializer import KnowledgeBaseInitializer
from DeepHSCode.knowledge.manager import KnowledgeBaseManager
from DeepHSCode.knowledge.progress_tracker import ProgressStage, ProgressTracker
from DeepHSCode.services.rag.components.routing import FileTypeRouter
from DeepHSCode.services.rag.factory import DEFAULT_PROVIDER, has_pipeline, normalize_provider_name
from DeepHSCode.utils.document_validator import DocumentValidator
from DeepHSCode.utils.error_utils import format_exception_message

from DeepHSCode.logging import get_logger
from DeepHSCode.services.config import PROJECT_ROOT, load_config_with_main

# Initialize logger with config (gracefully handle missing main.yaml on first run)
try:
...
def _validate_registered_provider(raw_provider: str | None) -> str:
    ...
    if not has_pipeline(candidate):
        from DeepHSCode.services.rag.service import RAGService
...
async def get_all_kb_configs():
    ...
    try:
        from DeepHSCode.services.config import get_kb_config_service
